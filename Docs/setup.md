...
1.  **Initialize and Start**:
    ...
    *   *What happens?* 
        ...
        *   Automatically creates the `rawg-lake` bucket in MinIO.
...
2.  Click `daily_football_leagues_update`.
3.  Click **Materialize All**.
4.  Click the **Partition Icon** (top right of the launch modal) and select yesterday's date.
5.  Click **Launch Run**.

**What happens next?**
1.  **Ingestion**: Python scripts fetch JSON for that date and upload it to MinIO (`s3://rawg-lake/raw/...`).
...
Since DuckDB runs inside the container, the database file is persisted at `local_ops/data/warehouse/game_analytics.duckdb`.
...
*   **"Bucket does not exist"**: The `create_buckets` container usually handles this. If it failed, log into MinIO Console (localhost:9001) and create a bucket named `rawg-lake` manually.
